...
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" <<-EOSQL
    ...
    -- Shared with wingtrill-notifier, which stores its own fcm_tokens
    -- table here via its own migrations (src/rdbms/notifier_migrate.erl).
    CREATE DATABASE wingtrill_notifier OWNER mongooseim;
EOSQL
